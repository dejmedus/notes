...
- `option + click` move cursor inside line
...
- `man commandName` info on command _man find_ **q to quit**
- `whatis commandName` simplified info on command
- `whereis executableName` returns path to executable _whereis python3_
...
- `find . -name <file-name> -delete` deletes all files (even nested) of filename
- `cat *.txt > combined.txt` combines all files in directory into one file
...
- `lsof -i :3000` checks what is running/listening on port
- `kill -15 PID` SIGTERM the process (ask politely)
- `kill -3 PID` SIGQUIT the process (write memory to file for debugging)
- `kill -9 PID` SIGKILL the process (nuclear, no cleanup)
- `killall processName` ends all processes with name

#### Standard Streams

The shell communicates through **3 standard streams**:

- `stdin (0)` standard input (keyboard or file input)
- `stdout (1)` standard output (program output)
- `stderr (2)` standard error (error messages)

The shell can be asked to print outside the terminal

- `command > file.txt` redirect stdout
- `command < input.txt` read in stdin
- `command 2> errors.txt` redirect stderr
- `command 2>/dev/null` redirect errors to black hole
- `command &> output.txt` redirect both stdout and stderr (eq `command > output.txt 2>&1`)

#### Exit Codes

Every program finishes with an **exit code**
...
- `printenv` or `env` shows all environment variables
- `echo $VAR_NAME` prints value of a variable
- `export VAR_NAME=value` sets a variable for current shell session
- `unset VAR_NAME` removes a variable
- `VAR_NAME=value command` run a command with a temporary variable
...
- `export DEBUG=true` set variable for current session
- `DEBUG=true node app.js` run program with variable set temporarily
- `unset DEBUG` remove variable
...
- `echo $PATH` show directories searched for commands
- `export PATH=$PATH:/new/directory` add a directory until shell is closed
...
- [Over the Wire](https://overthewire.org/wargames/bandit/) games to learn shell basics
- [Awesome Shell](https://github.com/alebcay/awesome-shell) list of command-line frameworks, tool kits, and guides
