...
Ordered, fixed-size, immutable collections

```python
my_tuple = ("Alice", 25, "pink")
my_tuple[0]  # get
# tuples are immutable, so no add/remove
for item in my_tuple:  # loop
    print(item)
name, age, favorite_color = my_tuple
```
